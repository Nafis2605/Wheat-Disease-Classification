# -*- coding: utf-8 -*-
"""cvt_with_class_150.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1emRct294EPExMmI7PHCH9nTe5F4r5CFo
"""
...
class CvT(nn.Module):
    def __init__(self, num_classes, image_size=224, patch_size=16, embedding_dim=128, num_heads=8, mlp_dim=256, dropout_rate=0.1):
        ...
        self.proj = nn.Linear(1024, embedding_dim)
        self.attention = nn.MultiheadAttention(embed_dim=embedding_dim, num_heads=num_heads, dropout=dropout_rate)
...
model = CvT(num_classes=12).to(device)
...
os.makedirs('/content/drive/MyDrive/dm project/cvt', exist_ok=True)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), '/content/drive/MyDrive/dm project/cvt/cvtwwc_best_model_150.pth')
    else:
        patience_counter += 1
...
with open('/content/drive/MyDrive/dm project/cvt/cvtwwc_training_history.pkl', 'wb') as f:
...
plt.tight_layout()
plt.savefig('/content/drive/MyDrive/dm project/cvt/cvtwwc_loss_acc_plot.png')
plt.show()
...
model = CvT(num_classes=len(class_names)).to(device)
model.load_state_dict(torch.load('/content/drive/MyDrive/dm project/cvt/cvtwwc_best_model_150.pth'))
model.eval()
...
plt.ylabel('True')
plt.savefig('/content/drive/MyDrive/dm project/cvt/cvtwwc_confusion_matrix_150.png')
plt.show()
...
plt.xlabel('Predicted')
plt.ylabel('True')
plt.savefig('/content/drive/MyDrive/dm project/cvt/cvtwwc_n_con_mat_150.png')
plt.show()
...
plt.legend()
plt.savefig('/content/drive/MyDrive/dm project/cvt/cvtwwc_roc_curve_150.png')
plt.show()
...
# model = CvT(num_classes=len(class_names)).to(device)
safe_load_model('/content/drive/MyDrive/dm project/cvt/cvtwwc_best_model_150.pth', model)
